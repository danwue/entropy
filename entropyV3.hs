...
patternScore xs = sum $ map score $ filter isPalindrome (continuousSubSeqs xs)
  where score [_] = 0 -- single counters do not give any score
        ...
        score [_, _, _] = 3 -- palindromes of length 3 are guaranteed to give 3 points, even if middle counter is missing
        score xs = length . filter (/= 0) $ xs -- otherwise, just count all the counters on the squares
